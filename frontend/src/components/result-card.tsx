...
function EmptyState() {
  return (
    <div className="text-center space-y-4">
      ...
      <div>
        <h3 className="text-[15px] font-semibold text-black">准备就绪</h3>
...
function LoadingState({ message }: { message: string }) {
  return (
    <div className="text-center space-y-4">
      ...
      <div>
        <h3 className="text-[15px] font-semibold text-black">{message}</h3>
        <p className="text-[13px] text-black/40 mt-1">请稍候...</p>
      </div>
    </div>
  );
}
...
function SuccessState({ downloadUrl, formatDescription, formatExtension, onReset }: SuccessStateProps) {
  return (
    <div className="text-center space-y-6 w-full max-w-xs">
      ...
      <div>
        <h3 className="text-[17px] font-semibold text-black">生成成功</h3>
      ...
      <div className="space-y-3">
        <a
          href={downloadUrl}
          className="flex items-center justify-center gap-2 w-full bg-black hover:bg-black/90 text-white font-medium text-[14px] py-3 px-5 rounded-xl transition-all shadow-sm hover:shadow active:scale-[0.98]"
        >
        ...
        <button
          onClick={onReset}
          className="text-[13px] text-black/40 hover:text-black transition-colors flex items-center justify-center gap-2 w-full py-2"
        >
